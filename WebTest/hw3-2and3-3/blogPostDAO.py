...
class BlogPostDAO:
    ...
    def get_post_by_permalink(self, permalink):
        ...
        post = self.posts.find_one({'permalink' : {'$eq' :permalink}})
    ...
    def add_comment(self, permalink, name, email, body):
        ...
        try:
            # XXX HW 3.3 Work here to add the comment to th`e designated post. When done, modify the line below to return the number of documents updated by your modification, rather than just -1.
            returnRecord  = self.posts.update_one({'permalink': permalink}, {'$addToSet' : {'comments' : comment}},upsert=True)
            print("sunil return record", returnRecord.modified_count)
            return returnRecord.modified_count
